...
static void encode_b_rt(VP9_COMP *cpi, ThreadData *td,
	...
	//Ehab: This function sets the edges of macroblock
	// x->e_mbd->mb_to_top_edge = -((mi_row * MI_SIZE) * 8);
	// x->e_mbd->mb_to_bottom_edge = ((mi_rows - bh - mi_row) * MI_SIZE) * 8;
	// x->e_mbd->mb_to_left_edge = -((mi_col * MI_SIZE) * 8);
	// x->e_mbd->mb_to_right_edge = ((mi_cols - bw - mi_col) * MI_SIZE) * 8;
...
static void nonrd_use_partition(VP9_COMP *cpi, ThreadData *td,
	...
	default:
		...
		assert(partition == PARTITION_SPLIT);
		subsize = get_subsize(bsize, PARTITION_SPLIT);
		// Ehab this will split the 64x64 block into 8x8
		/*
			#define BLOCK_4X4 0
			#define BLOCK_4X8 1
			#define BLOCK_8X4 2
...
void mem_put_t(void *vmem, int val) {
...
static void encode_nonrd_sb_row_ehab(VP9_COMP *cpi, ThreadData *td,
		TileDataEnc *tile_data, int mi_row,
		TOKENEXTRA **tp, size_t *size, uint8_t *dest) {
	...
	//printf("sb_row: %d, num_sb_cols:%d, col_st:%d, col_en:%d, blockSize: %d\n",sb_row,num_sb_cols, mi_col_start,mi_col_end,MI_BLOCK_SIZE);

	// Initialize the left context for the new SB row
	memset(&xd->left_context, 0, sizeof(xd->left_context));
	memset(xd->left_seg_context, 0, sizeof(xd->left_seg_context));

	//printf("s_row: %d, e_row: %d, s_col: %d, e_col: %d, blockSize:%d\n",tile_info->mi_row_start,tile_info->mi_row_end,tile_info->mi_col_start,tile_info->mi_col_end, MI_BLOCK_SIZE);

	// Ehab : code each sublock over column!
	for (mi_col = mi_col_start, sb_col_in_tile = 0; mi_col < mi_col_end;
			mi_col += MI_BLOCK_SIZE, ++sb_col_in_tile) {
		tokStart=*tp;
		...
		//Store token which is tp
		//printf("mi_col:%d\n",mi_col);
		//printf("pointer:%p\n",tokStart);

		//printf("row:%d, col:%d,			 st row:%3d, st col:%3d\n",mi_row,mi_col, mi_row*8, mi_col*8);
		const struct segmentation *const seg = &cm->seg;
		RD_COST dummy_rdc;
		const int idx_str = cm->mi_stride * mi_row + mi_col;
		MODE_INFO **mi = cm->mi_grid_visible + idx_str;
		...
		//printf("partition_search_type%d\n",partition_search_type);
		...
		//sbInWidth = (ceil(cpi->Source->y_width/64.0));
		//sbInHeigth = (ceil(cpi->Source->y_height/64.0));
		...
		if(num==0) // for the first superblock within the frame
		{
			...
			vpx_start_encode(&residual_bc, dest ); //initialize the range encoder.
		...
		if(num%sbInWidth==0 ) //image width
		{
			sbefore = *size - sbefore;
			...
			int st = (mi_row-8)*8;
			int en = (mi_row)*8;
			if (en > cpi->Source->y_height)
				en = cpi->Source->y_height;
			if(num!=0)
				printf("EncodeTime rows(%d,%d) : %d ms; size:%d B \n",st,en, msec,sbefore);
		...
		memcpy(xd->left_seg_context,leftSeg, sizeof(xd->left_seg_context));

		PARTITION_CONTEXT * temp = xd->above_seg_context;
		xd->above_seg_context=aboveSeg;

		int iq=residual_bc.pos;
		write_modes_sb2(cpi, xd, tile_info, &residual_bc, &tokStart, *tp, mi_row,mi_col,
				BLOCK_64X64,  &cpi->max_mv_magnitude, cpi->interp_filter_selected);
		memcpy(leftSeg,xd->left_seg_context, sizeof(leftSeg));
		...
		if(num==(sbInHeigth*sbInWidth)) //image width/64  * heigth/64
		{
			sbefore = *size - sbefore;
		    ...
		    *size += residual_bc.pos- i;
			if (FLAG_WRITE ==1)
			{
				char hname[11];
				sprintf(hname, "%d",fnum);
				strcat(hname,"_end");
			...
			if(FLAG_WRITE ==1)
			{
				...
				char hname2[11];
				sprintf(hname2, "%d",fnum);
				strcat(hname2,"_h2");

				FILE *fp = fopen(hname2,"w");
		...
		/*if(num==(sbInHeigth*sbInWidth)) //image width/64  * heigth/64
		{
			cpi->tplist[0][0][16].stop = *tp;
			cpi->tplist[0][0][16].count =
					(unsigned int)(cpi->tplist[0][0][16].stop -
							cpi->tplist[0][0][16].start);
			//printf("%p,%p,%d\n",cpi->tplist[0][0][16].start,cpi->tplist[0][0][16].stop,cpi->tplist[0][0][16].count);


			vp9_pack_bitstream2(cpi, dest, size); // create the headers "Compressed and uncompressed"
			printf("ETotal Size: %ld B\n",*size);
...
static void encode_nonrd_sb_row(VP9_COMP *cpi, ThreadData *td,
	...
	for (mi_col = mi_col_start, sb_col_in_tile = 0; mi_col < mi_col_end;
		...
		{
			once = 0;
			dest = malloc(sizeof(uint8_t)*3110400);
			size = malloc(sizeof(size_t)*10);
		...
		if(num%sbInWidth==0 ) //image width
		{
			...
			if (FLAG_WRITE == 1)
			{
				...
				FILE *fp = fopen(cfileName, "w+");
				fsize+=residual_bc.pos;
				for(ssss;ssss<residual_bc.pos;ssss++)
				{
					char x = residual_bc.buffer[ssss];
					fwrite(&x, 1, sizeof(x), fp);
				}
...
void vp9_encode_sb_row_ehab(VP9_COMP *cpi, ThreadData *td, int tile_row,
		int tile_col, int mi_row, size_t *size, uint8_t *dest) {
	//printf("vp9_encode_sb_row()\n");
	VP9_COMMON *const cm = &cpi->common;
	const int tile_cols = 1 << cm->log2_tile_cols;
	TileDataEnc *this_tile = &cpi->tile_data[tile_row * tile_cols + tile_col];
	...
	if (cpi->sf.use_nonrd_pick_mode){
		//printf("encode_nonrd_sb_row()\n");
		encode_nonrd_sb_row_ehab(cpi, td, this_tile, mi_row, &tok, size, dest);}
#if !CONFIG_REALTIME_ONLY
	else{
	...
	//printf("h%p,%p,%d\n",cpi->tplist[tile_row][tile_col][tile_sb_row].start,cpi->tplist[tile_row][tile_col][tile_sb_row].stop,cpi->tplist[tile_row][tile_col][tile_sb_row].count);

	//printf("tile_row:%d, tile_col:%d, tile_sb_row:%d\n",tile_row,tile_col,tile_sb_row);
	//printf("count_a:%d\n",cpi->tplist[tile_row][tile_col][tile_sb_row].count);
	//printf("poiner:%p\n",cpi->tplist[tile_row][tile_col][tile_sb_row].start);
	//printf("poiner:%p\n",cpi->tplist[tile_row][tile_col][tile_sb_row].stop);
	/*if(tile_sb_row==0)
 {
	 char snum[10];
     sprintf(snum, "%d", framenum);
	 char * cfileName = snum;
	 frame=fopen(cfileName, "w+");
	 framenum+=1;
 }
 TOKENEXTRA *token = cpi->tplist[tile_row][tile_col][tile_sb_row].start;
 while(token!=cpi->tplist[tile_row][tile_col][tile_sb_row].stop)
 {
      fwrite(token, 1, sizeof(char), frame);
      token++;
 }
 fflush(frame);*/
...
void vp9_encode_tile_ehab(VP9_COMP *cpi, ThreadData *td, int tile_row,
		int tile_col, size_t *size, uint8_t *dest) {
	VP9_COMMON *const cm = &cpi->common;
	const int tile_cols = 1 << cm->log2_tile_cols;
	TileDataEnc *this_tile = &cpi->tile_data[tile_row * tile_cols + tile_col];
	const TileInfo *const tile_info = &this_tile->tile_info;
	...
	int mi_row;
	//encode the image superblock after another, the superblock is 64x64
	//this will loop will mainly goes 0->hight/64, so for 1920x1080, this will
	//go from 0-> 16 (1080/64 - 1), each row may contain multiple superblocks
	//there will be another loop to loop over the super block within the column
	// so on raw basis we have ~ 17 superblock, on the column basis we have
	// 30 (1920/64) superblocks, on total there will be 510 superblocks
	// this is the raw loop, the column loop in encode_nonrd_sb_row() function.
	for (mi_row = mi_row_start; mi_row < mi_row_end; mi_row += MI_BLOCK_SIZE)
		vp9_encode_sb_row_ehab(cpi, td, tile_row, tile_col, mi_row,size, dest);
}
...
static void encode_tiles_ehab(VP9_COMP *cpi,  size_t *size, uint8_t *dest) {
	VP9_COMMON *const cm = &cpi->common;
	const int tile_cols = 1 << cm->log2_tile_cols;
	const int tile_rows = 1 << cm->log2_tile_rows;
	int tile_col, tile_row;
	...
	for (tile_row = 0; tile_row < tile_rows; ++tile_row)
		for (tile_col = 0; tile_col < tile_cols; ++tile_col)
			vp9_encode_tile_ehab(cpi, &cpi->td, tile_row, tile_col, size, dest);
}
...
static void encode_frame_internal_ehab (VP9_COMP *cpi, size_t *size,  uint8_t *dest)
{
	...
	//Ehab this function copies the data to MBs.
...

	{
		...
		if (!cpi->row_mt) {
			...
			if (VPXMIN(cpi->oxcf.max_threads, 1 << cm->log2_tile_cols) > 1){    printf("vp9_encode_tiles_mt()\n");
			vp9_encode_tiles_mt(cpi);}
			else{
				encode_tiles_ehab(cpi, size , dest);}
		}
		else {
...
void vp9_encode_frame_ehab(VP9_COMP *cpi, size_t *size,  uint8_t *dest)
{
	...
	if (cpi->sf.frame_parameter_update) {
		...
		encode_frame_internal_ehab(cpi, size, dest);
...
static void encode_superblock(VP9_COMP *cpi, ThreadData *td, TOKENEXTRA **t,
	...
	MACROBLOCK *const x = &td->mb;
	MACROBLOCKD *const xd = &x->e_mbd;
	//printf("encode_superblock, mi_row:%d, mi_col:%d, blockSize:%d\n",mi_col,mi_row,bsize);
	...
	if (!is_inter_block(mi))  //INTRA Prediction (I-frames)
	{
		int plane;
		...
		mi->skip = 1;
		//Ehab: encode each plane separatly
		for (plane = 0; plane < MAX_MB_PLANE; ++plane)
			vp9_encode_intra_block_plane(x, VPXMAX(bsize, BLOCK_8X8), plane, 1); //Ehab: this function
	...
	else { //INTER Prediction (P-frames)
		int ref;
		...
		vp9_encode_sb(x, VPXMAX(bsize, BLOCK_8X8), mi_row, mi_col, output_enabled); //Ehab
		vp9_tokenize_sb(cpi, td, t, !output_enabled, seg_skip,
				VPXMAX(bsize, BLOCK_8X8));
	}
