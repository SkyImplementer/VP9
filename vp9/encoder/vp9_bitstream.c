...
void write_modes_sb2(
  ...
  if (subsize < BLOCK_8X8) {
    ...
    switch (partition) {
      ...
      default:
        assert(partition == PARTITION_SPLIT);
        write_modes_sb2(cpi, xd, tile, w, tok, tok_end, mi_row, mi_col, subsize,
                       max_mv_magnitude, interp_filter_selected);
        write_modes_sb2(cpi, xd, tile, w, tok, tok_end, mi_row, mi_col + bs,
                       subsize, max_mv_magnitude, interp_filter_selected);
        write_modes_sb2(cpi, xd, tile, w, tok, tok_end, mi_row + bs, mi_col,
                       subsize, max_mv_magnitude, interp_filter_selected);
        write_modes_sb2(cpi, xd, tile, w, tok, tok_end, mi_row + bs, mi_col + bs,
                       subsize, max_mv_magnitude, interp_filter_selected);
        break;
    }
  }
...
//printf("===========\n");
  //printf("row:%d, col:%d, pos:%d, low:%d, range:%d, count:%d\n",mi_row, mi_col, w->pos, w->lowvalue, w->range, w->count);
  ...
  for (tx_size = TX_4X4; tx_size <= max_tx_size; ++tx_size) {
    ...
    if (cpi->td.counts->tx.tx_totals[tx_size] <= 20 ||
      ...
      vpx_write_bit(w, 0);
    	...
    	//EHAB
    	//old probabilities are stored at cpi->common.fc-> coef_probs
    	//new probabilities are stored at frame_coef_probs (being build in the next line)
...
static void encode_loopfilter2(struct loopfilter *lf,
                              struct vpx_write_bit_buffer *wb) {
  int i;
...
void write_uncompressed_header2(VP9_COMP *cpi,
                                      struct vpx_write_bit_buffer *wb) {
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCKD *const xd = &cpi->td.mb.e_mbd;
...
void vp9_pack_bitstream2(VP9_COMP *cpi, uint8_t *dest, size_t *size) {
  ...
  char hname[11];
  sprintf(hname, "%d",fnumx);
  strcat(hname,"_header");
  fnumx++;
