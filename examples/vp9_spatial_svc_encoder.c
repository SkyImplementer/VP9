...
static const uint32_t default_timebase_den = 30;
static const uint32_t default_bitrate = 1000;
static const uint32_t default_spatial_layers = 5;
static const uint32_t default_temporal_layers = 1;
static const uint32_t default_kf_dist = 30;
...
static void parse_command_line(int argc, const char **argv_,
	...
	//enc_cfg->g_error_resilient = 1; //EHAB
...
int main(int argc, const char **argv) {
	...
	while (!end_of_stream) {
		...
		while ((cx_pkt = vpx_codec_get_cx_data(&encoder, &iter)) != NULL) {
			uint8_t *buffer = (uint8_t *) cx_pkt->data.frame.buf;
